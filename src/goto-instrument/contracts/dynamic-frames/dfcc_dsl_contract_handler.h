...
#ifndef CPROVER_GOTO_INSTRUMENT_CONTRACTS_DFCC_DSL_CONTRACT_HANDLER_H
#define CPROVER_GOTO_INSTRUMENT_CONTRACTS_DFCC_DSL_CONTRACT_HANDLER_H
...
public:
  ...
  /// signature compatible with that of "contract_id".
  /// Returns true if found, false if "contract::contract_id" does not exist or
  /// if it exists but has signature incompatible with that of "contract_id".
...
protected:
  ...
  /// Searches for a symbol named "contract::contract_id" that has a type
  /// signature compatible with that of "contract_id".
  /// Returns a pointer to the symbol if found, or nullptr if
  /// "contract::contract_id" does not exist or if it exists but has signature
  /// incompatible with that of "contract_id".
  const symbolt *get_pure_contract_symbol_ptr(const irep_idt &contract_id);

  /// Searches for a symbol named "contract::contract_id" that has a type
  /// signature compatible with that of "contract_id".
  /// Returns a reference to the symbol if found, throws an error if
  /// "contract::contract_id" does not exist or if it exists but has signature
  /// incompatible with that of "contract_id".

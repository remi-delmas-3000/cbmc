  ...
  cfg_infot &cfg_info)
{
  ...
  payload.add(goto_programt::make_function_call(
    code_function_callt{
      library.dfcc_fun_symbol[dfcc_funt::WRITE_SET_REMOVE_DEAD].symbol_expr(),
      {write_set, address_of_exprt(decl_symbol)}},
    target->source_location()));
  ...
  const cfg_infot &cfg_info)
{
  ...
  if(can_cast_expr<symbol_exprt>(lhs))
  {
    ...
    check_source_location.set_comment(
      "Assignment to local symbol " +
      from_expr_using_mode(ns, language_mode, lhs) + " (always allowed)");
  ...
  else
  {
    ...
    {
      check_source_location.set_comment(
        "Assignment to local symbol composite member " +
        from_expr_using_mode(ns, language_mode, lhs) + " implicitly allowed");
  ...
  cfg_infot &cfg_info)
{
       ...
       target->source_location(), check_source_location, mode, lhs, cfg_info))
  {
    ...
    std::string comment = "Check that " + from_expr_using_mode(ns, mode, lhs) +
  ...
  cfg_infot &cfg_info)
{
  ...
  if(dead_ptr != nullptr)
  {
    ...
    payload.add(goto_programt::make_function_call(
      code_function_callt{
        library.dfcc_fun_symbol[dfcc_funt::WRITE_SET_REMOVE_DEALLOCATED]
          .symbol_expr(),
        {write_set, *dead_ptr}},
      target_location));
  ...
  goto_programt &goto_program)
{
  ...
  // CALL remove_deallocated(write_set, lhs);
  // skip_target: SKIP;
  // ----
  ...
  std::string comment = "Check that deallocation of " +
                        from_expr_using_mode(ns, mode, ptr) +
                        " is allowed by the frees clause";

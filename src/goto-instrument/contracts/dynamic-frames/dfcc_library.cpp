...
static const std::map<dfcc_funt, irep_idt> dfcc_fun_to_name = {
  ...
  {dfcc_funt::WRITE_SET_REMOVE_DEAD,
    CONTRACTS_PREFIX "write_set_remove_dead"},
  {dfcc_funt::WRITE_SET_REMOVE_DEALLOCATED,
    CONTRACTS_PREFIX "write_set_remove_deallocated"},
  ...
  std::set<irep_idt> &to_instrument)
{
  ...
  missing.insert("__CPROVER_deallocate");
  to_instrument.insert("__CPROVER_deallocate");

  // Make sure all front end functions are loaded
  missing.insert("__CPROVER_assignable");
  missing.insert("__CPROVER_object_from");
  missing.insert("__CPROVER_object_upto");
  missing.insert("__CPROVER_whole_object");
  missing.insert("__CPROVER_freeable");
}
...
static const std::map<dfcc_funt, int> to_unwind = {
  {dfcc_funt::WRITE_SET_REMOVE_DEALLOCATED, 0},

...
#pragma CPROVER check disable "conversion"
  ...
  __CPROVER_assert(
    __CPROVER_same_object(car.lb, car.ub), "no offset bits overflow on ub");
...
#pragma CPROVER check disable "conversion"
  ...
  elem->size = size;
  elem->lb = ptr;
  // this is meant to be equivalent to using pointer-overflow checks
  // if this passes then the pointer arithmetic for car.ub will not
  // overflow into object bits
  __CPROVER_assert(
  ...
  if(offset > 0)
  {
    __CPROVER_assert(
      ...
      "no pointer overflow on car.ub computation");
  }
  elem->ub = (void *)((char *)ptr + size);
  __CPROVER_size_t idx_p1 = idx + 1;
  __CPROVER_size_t wm = set->watermark;
  set->watermark = idx_p1 > wm ? idx_p1 : wm;
#pragma CPROVER check pop
}
  ...
  void *ptr)
{
  ...
  __CPROVER_size_t object_id = __CPROVER_POINTER_OBJECT(ptr);
  set->watermark =
    object_id + 1 >= set->watermark ? object_id + 1 : set->watermark;
  set->nof_elems =
...
void __CPROVER_contracts_write_set_insert_whole_object(
  ...
  void *ptr)
{
  void *whole_object_start = ((char *)ptr) - __CPROVER_POINTER_OFFSET(ptr);
  __CPROVER_size_t whole_object_size = __CPROVER_OBJECT_SIZE(ptr);
  __CPROVER_contracts_car_set_insert(
    &(set->contract_assigns), idx, whole_object_start, whole_object_size);
}
...
/* FUNCTION: __CPROVER_contracts_write_set_remove_dead */
...
void __CPROVER_contracts_write_set_remove_dead(
  __CPROVER_contracts_write_set_ptr_t set,
  void *ptr)
{
...
/* FUNCTION: __CPROVER_contracts_write_set_remove_deallocated */
...
void __CPROVER_contracts_write_set_remove_deallocated(
  __CPROVER_contracts_write_set_ptr_t set,
  void *ptr)
{
...
SET_CHECK_ASSIGNMENT_LOOP:
  ...
  {
    incl |=
      car.is_writable & elem->is_writable &
      __CPROVER_same_object(elem->lb, car.lb) &
      (__CPROVER_POINTER_OFFSET(elem->lb) <= __CPROVER_POINTER_OFFSET(car.lb)) &
      (__CPROVER_POINTER_OFFSET(car.ub) <= __CPROVER_POINTER_OFFSET(elem->ub));
    ++elem;
    --idx;
  }
  return incl;
  ...
  void *ptr)
{
  ...
  return __CPROVER_contracts_write_set_check_assignment(set, ptr, size);
}
...
#pragma CPROVER check pop
    ...
    if((ptr != 0) & preconditions & nondet_CPROVER_bool())
    {
      ...
      __CPROVER_contracts_write_set_remove_deallocated(set, ptr);
      // also record effects in the caller write set
      if(target != 0)
        __CPROVER_contracts_write_set_remove_deallocated(target, ptr);
    }
    --idx;
    ++current;
  }
